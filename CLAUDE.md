...
3. **Load the patient in your platform.** From the handle: `patient.yourId` (the id you gave us for this person last time, or null), `patient.name`, `patient.dob`. Find them (`byId(yourId) || byName(name)`); if they don't exist, offer to create them from what Comprehend knows. Then navigate your UI to that patient — open their chart. If `patient` is `null` (the visit has no patient yet), do not clear or change what your user has open; step 4 still runs for that patient.
4. **When your patient page has loaded, set context — always.** Call `comprehend.setContext(markdown, { id: yourPatientId, name: yourPatientName, dob })` with a short markdown summary of what you know (adherence, test results, program). This is the one method; call it whether or not Comprehend has a patient on the visit. With one, it is also what links the two records (next visit `patient.yourId` arrives set). With none, Comprehend shows the clinician a one-click "Use <your app>'s patient for this visit"; on click the patient is found or created, assigned, and you receive `patient(changed)` with `yourId` — run the same handler again. Don't build your own assign; nothing happens without the clinician's click. Pass `dob` too when you have it. If the two records don't look like the same person (no prior link, DOBs don't match or aren't known, names don't resemble — "Robert" vs "Bob" counts), Comprehend rejects it and you get `error({ code: 'CONTEXT_REJECTED' })`. **The clinician resolves this in Comprehend's UI**, not yours: they see both names and can confirm "same patient" once; you then receive a fresh `patient` event with `yourId` set and simply run the same code path again. Don't build your own override.
...
Keep your page at its natural height (no fixed-height body with its own scroll); the script reports content height and Comprehend sizes the frame. The frame never shows scrollbars, so nothing may overflow sideways: fit 350 px, and avoid `min-height: 100vh` (the frame is sized to your document, so a viewport-relative minimum feeds back into the measurement).
...
| Surface | Detail |
|---|---|
| `comprehend.on('patient', fn)` | `fn(patient \| null, { reason })`. `patient` is plain data: `{ id, name, dob, yourId }`. |
| `comprehend.setContext(md, { id, name, dob })` / `comprehend.clearContext()` | The one write. Works with or without a Comprehend patient (see lifecycle step 4). |
