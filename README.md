...
**The manifest** — host it at `https://<your origin>/.well-known/comprehend-app.json`. Every key is optional; without the file admins see your hostname as the name and the pasted link is used as-is. `launchUrl` must be on the same origin as the manifest. The sandbox's *Check my app* shows exactly what admins will see. Filled-in examples for the two example apps are in [`vendor-examples/.well-known/`](vendor-examples/.well-known/) — with a note on why they sit there rather than at the root.
